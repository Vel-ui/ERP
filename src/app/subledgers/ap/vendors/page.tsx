...
const initialVendors: Vendor[] = [
  { id: "V-001", name: "Amazon Web Services", pointPerson: "Sarah Chen", email: "billing@aws.amazon.com", paymentMethod: "Bank Transfer", paymentTerms: "Net 30", is1099: false, status: "active", address: { country: "US", city: "Seattle", state: "WA", zip: "98109" }, taxId: "91-1234567", defaultAccount: "6100 - Cloud Hosting", department: "Engineering", customerSegment: "Technology" },
  { id: "V-002", name: "Gusto Inc.", pointPerson: "Mike Rodriguez", email: "ap@gusto.com", paymentMethod: "Bank Transfer", paymentTerms: "Net 15", is1099: false, status: "active", address: { country: "US", city: "San Francisco", state: "CA", zip: "94105" }, taxId: "46-5678901", defaultAccount: "6200 - Salaries", department: "Human Resources", customerSegment: "Services" },
  { id: "V-003", name: "Google Cloud", pointPerson: "Lisa Park", email: "cloud-billing@google.com", paymentMethod: "Credit Card", paymentTerms: "Net 30", is1099: false, status: "active", address: { country: "US", city: "Mountain View", state: "CA", zip: "94043" }, taxId: "77-2345678", defaultAccount: "6300 - Software", department: "Engineering", customerSegment: "Technology" },
  { id: "V-004", name: "WeWork", pointPerson: "James Kim", email: "invoices@wework.com", paymentMethod: "Bank Transfer", paymentTerms: "Net 30", is1099: false, status: "active", address: { country: "US", city: "New York", state: "NY", zip: "10001" }, taxId: "20-3456789", defaultAccount: "6500 - Rent", department: "Operations", customerSegment: "Real Estate" },
  { id: "V-005", name: "Johnson Legal LLP", pointPerson: "Patricia Johnson", email: "billing@johnsonlegal.com", paymentMethod: "Bank Transfer", paymentTerms: "Net 45", is1099: true, status: "active", address: { country: "US", city: "Chicago", state: "IL", zip: "60601" }, taxId: "36-4567890", defaultAccount: "6600 - Legal Fees", department: "Legal", customerSegment: "Professional Services" },
  { id: "V-006", name: "Staples Inc.", pointPerson: "Robert Lee", email: "corporate@staples.com", paymentMethod: "Credit Card", paymentTerms: "Due on Receipt", is1099: false, status: "inactive", address: { country: "US", city: "Framingham", state: "MA", zip: "01702" }, taxId: "04-5678901", defaultAccount: "6400 - Office Supplies", department: "Operations", customerSegment: "Retail" },
];
...
export default function VendorsPage() {
  ...
  function handleAddVendor() {
    ...
    const newVendor: Vendor = {
      ...
      name: formName, pointPerson: formPerson, email: formEmail,
      ...
      is1099: formIs1099, status: "active",
    ...
    <div style={{ padding: 24, background: '#f9f9f9' }}>
      <div style={{ maxWidth: '80rem', margin: '0 auto' }}>
        <div className="flex justify-between items-start mb-6">
          <div>
            <h1 className="mx-h1">Vendors</h1>
            <p className="mx-text-secondary mt-1">{vendors.length} vendors total</p>
          </div>
          <Button onClick={() => setModalOpen(true)}><Plus size={16} className="mr-1" /> Add Vendor</Button>
        </div>

        <div className="mb-4" style={{ maxWidth: 320 }}>
          <Input placeholder="Search vendors…" value={searchText} onChange={(e) => setSearchText(e.target.value)} />
        </div>
          ...
          <table className="mx-table">
            ...
            <tbody>
              {filteredVendors.map((v) => (
                <tr key={v.id}>
                  <td style={{ fontWeight: 500, color: '#2D2926' }}>{v.name}</td>
                  <td>{v.pointPerson}</td>
                  <td>{v.email}</td>
                  <td>{v.paymentMethod}</td>
                  <td>{v.paymentTerms}</td>
                  <td style={{ textAlign: 'center' }}>
                    {v.is1099 ? <Tag variant="processing">Yes</Tag> : <span className="mx-text-secondary" style={{ fontSize: 12 }}>No</span>}
                  </td>
                  <td style={{ textAlign: 'center' }}>
                    <Tag variant={v.status === "active" ? "success" : "default"}>
              ...
              {filteredVendors.length === 0 && (
                <tr><td colSpan={7} style={{ textAlign: 'center', padding: 32 }} className="mx-text-secondary">No vendors found</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <Modal isOpen={modalOpen} onClose={() => { setModalOpen(false); resetForm(); }} title="Add Vendor" size="lg">
          <div className="space-y-5" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
            ...
            <div>
              <p className="mx-text-secondary" style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>Address</p>
              <div className="grid grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="mx-label">1099 Eligible</label>
                <button
                  type="button"
                  onClick={() => setFormIs1099(!formIs1099)}
                  style={{ position: 'relative', display: 'inline-flex', height: 24, width: 44, alignItems: 'center', borderRadius: 12, background: formIs1099 ? '#154738' : '#E9E9E9', transition: 'background 0.2s', border: 'none', cursor: 'pointer' }}
                >
                  <span style={{ display: 'inline-block', height: 16, width: 16, borderRadius: '50%', background: '#fff', transition: 'transform 0.2s', transform: formIs1099 ? 'translateX(24px)' : 'translateX(4px)' }} />
                </button>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
